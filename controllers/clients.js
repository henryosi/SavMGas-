...
function getNearDriver(req, res) {
    ...
    G_User.findById(client_id, function (err, user) {
        if(err) {
                                            ...
                                            if(err) {
                                                ...
                                                request({
                                                        ...
                                                        "headers": { //We can define headers too
                                                            ...
                                                            'Authorization': 'key=AIzaSyBr6_kLRRLByjUJPE1kH83fmGhN5uA0KjY'//get key
                                                        },
                                                        body: JSON.stringify(
                                                            {
                                                    ...
                                                    , function (error, response, body) {
                                                        if(error) {
                                                            res.json({
                                                                valid: false,
                                                                status: "notify",
                                                                message: "Ride confirmed but the driver not notified yet"
                                                            });
                                                        }
                                                        else {
                                                            res.json({
...
function getRidesHistory(req, res) {
    ...
    if(clientId) {
        Client.findById(clientId, function(err, client){
            if(err) {
                ...
                G_User.findOne({_id: client.personalData}, 'rides', function (err, g_user) {
                    if(err) {
                        res.json({
                            valid: false,
